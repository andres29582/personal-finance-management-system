...
O backend possui **72 handlers HTTP reais** em controllers, e o `backendnest/swagger.yaml` foi atualizado para documentar **72 operacoes**. A validacao mecanica confirmou a paridade entre controllers e Swagger.
...
| Planejamentos | 12 | 12 | Coberto |
| **Total** | **72** | **72** | **Coberto** |

Validacao mecanica executada:

- `rg '@(Get|Post|Patch|Delete|Put)\(' backendnest/src -g '*.controller.ts'`: 72 handlers.
- `rg '^\s{4}(get|post|patch|delete|put):' backendnest/swagger.yaml`: 72 operacoes.
- `rg '^\s+operationId:' backendnest/swagger.yaml`: 72 operationIds.
- Parse YAML com Python/PyYAML: OK.
- Validacao interna de `$ref`: 416 refs, 0 refs ausentes.
...
| --- | --- | --- |
| C-01 | Swagger documentava 43 operacoes para 72 handlers reais. | `backendnest/swagger.yaml` agora documenta 72 operacoes. |
...
| I-07 | Operacoes nao tinham `operationId`, prejudicando geracao de clientes/SDKs. | As 72 operacoes agora possuem `operationId` unico e estavel. |
...
| Planejamentos | 12 handlers implementados | Swagger documenta os 12 handlers reais; spec conceitual segue complementar | Coerente no contrato oficial |
...
| Cobertura | 72 operacoes para 72 handlers reais | N/A | Swagger completo frente aos controllers |
...
| --- | --- | --- | --- |
| Backend | `npm test` | Passou: 55 suites, 315 tests | Primeira tentativa no sandbox falhou com `spawn EPERM`; passou fora do sandbox. |
...
| OpenAPI | Parse YAML, refs, path params, seguranca, contagem e compatibilidade 3.0 | Passou | 72 operacoes, 72 operationIds, 416 refs resolvidas, 0 refs ausentes, 0 erros de path params, 0 erros de seguranca, 0 `exclusiveMinimum` numerico. |
